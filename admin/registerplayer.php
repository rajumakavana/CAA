...
<body>
    ...
    <ul class="sidebar-nav" id="sidebar-nav">
      ...
      <li class="nav-item">
        ...
        <ul id="tables-nav" class="nav-content collapse show" data-bs-parent="#sidebar-nav">
          <li>
            <a href="registerplayer.php" class="active">
  ...
  <main id="main" class="main">
    ...
    <div class="pagetitle">
      ...
      <nav>
        <ol class="breadcrumb">
          ...
          <li class="breadcrumb-item active">Registration Table</li>
        </ol>
      </nav>
    </div><!-- End Page Title -->
    ...
    <section class="section">
      ...
      <div class="text-center" style="color: #007BFF; font-weight:bold; "><i class="fas fa-user"></i> &nbsp;Player Details</div>
      <!-- Dark Table -->
      ...
      <table class="table table-dark table-hover">
        <thead>
          <tr>
            ...
            <th scope="col">Bowl-Type</th>
            <th scope="col">First-Pref.</th>
            <th scope="col">Captain Exp.</th>
        ...
        <?php
            if(isset($_POST['find']))
              {
                  $filter=$_POST['search'];
                  $sql="SELECT * FROM `regplayer` WHERE `id` like '%$filter%' or `first_name` LIKE '%$filter%' or `last_name` LIKE '%$filter%' or `father_name` LIKE '%$filter%' or `mother_name` LIKE '%$filter%' or `mobile` LIKE '%$filter%' or `email` LIKE '%$filter%' or `district` LIKE '%$filter%' or `state` LIKE '%$filter%' or `country` LIKE '%$filter%';";
                  ...
                  echo "<td> $row[21]</td>";
                  echo "<td> $row[22]</td>";
                  echo "<td><img src='../$row[25]' width='40px' height='40px'> </td>";
                  echo "<td><a class='btn btn-success' href='approval.php?id=$row[0]'><i class='bi bi-check-lg'></i></a></td>";
                  echo "<td><a class='btn btn-danger' href='regpdel.php?id=$row[0]'><i class='bi bi-trash'></i></a></td>";
                  echo "<td><a class='btn btn-warning' href='../pprint.php?id=$row[0]'><i class='bi bi-download'></i></a></td>";
                  echo "</tr>";
                      }
                    }
                    else
                    {
                      echo "<td colspan='28' align='center'>No Record Available</td>";
                      echo "</tr>";
                    }
                }
                else{
