...
<body>
  ...
  <main id="main" class="main">
    ...
    <div class="text-center" style="color: #007BFF; font-weight:bold; "><i class="fas fa-user"></i> &nbsp;Contact Details</div>
      <!-- Dark Table -->
      ...
      <table class="table table-danger table-hover">
        ...
        <?php
            ...
            {
                $filter=$_POST['search'];
                $sql="SELECT * FROM `contact` WHERE `id` like '%$filter%' or `name` LIKE '%$filter%' or  `email` LIKE '%$filter%'  or `subject` LIKE '%$filter%';";
                ...
                if(mysqli_num_rows($qry)>0)
                {
                    while($row=mysqli_fetch_array($qry))
                    {
                      ...
                      echo "<td> $row[3]</td>";
                      echo "<td> $row[4]</td>";
                      echo "<td><a class='btn btn-danger' href='condel.php?id=$row[0]'><i class='bi bi-trash'></i></a></td>";
                      echo "</tr>";
                    }
                  }
                  else
                  {
                    echo "<td colspan='6' align='center'>No Data Available</td>";
                      echo "</tr>";
                  }
                }
                else
